...
public class Callback implements Serializable {
    ...
    private void initializeIntent() {
        if (intentPackageName != null) {
            intent = new Intent();
            intent.setComponent(new ComponentName(intentPackageName, "com.connecthings.altbeacon.beacon.BeaconIntentProcessor"));
        }
    }
    ...
    public boolean call(Context context, String dataName, Parcelable data) {
        LogManager.d(TAG, "call - intent: %s, intentPackageName: %s", intent, intentPackageName);
        if(intent == null){
            initializeIntent();
        }
        if (intent != null) {
